...
6. **Memoria estructurada (obligatorio):** ejecutar `memory-store --file agents/jarvis/memory.json format-prompt` (desde raíz `jarvis-ecosystem/`, bin en `skills/global/memory-store/bin/`) e inyectar el markdown resultante al contexto operativo. Si el JSON está vacío, seguir con prosa (`MEMORY.md`) y proponer consolidación (`scripts/memory-consolidate.sh`).
