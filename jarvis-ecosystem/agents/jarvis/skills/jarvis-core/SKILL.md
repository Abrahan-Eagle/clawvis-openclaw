...
Proyecto **nuevo o legacy sin JARVIS**: escribe **`init jarvis`** en el chat (skill `project-bootstrap-ops`, [PROJECT_ONBOARDING.md](file:///var/www/html/proyectos/AIPP/jarvis-skills-library/docs/PROJECT_ONBOARDING.md)) antes de planificar features.

Si el repo tiene `.kittify/`, ver `kitty-router` ([docs/SPEC_KITTY_INTEGRATION.md](file:///var/www/html/proyectos/AIPP/jarvis-skills-library/docs/SPEC_KITTY_INTEGRATION.md)) — **no** `speckit-*`.

Si el repo tiene `openspec/` (sin otros marcadores SDD), ver `openspec-router` ([docs/AWESOME_SPEC_KITS.md](file:///var/www/html/proyectos/AIPP/jarvis-skills-library/docs/AWESOME_SPEC_KITS.md)) — **no** `speckit-*`.

Si el repo tiene `.specify/` (sin `.kittify/` ni `openspec/`), ver `sdd-router` y cadena `speckit-*` ([docs/SDD_SPECKIT_INTEGRATION.md](file:///var/www/html/proyectos/AIPP/jarvis-skills-library/docs/SDD_SPECKIT_INTEGRATION.md)).

Para UI/UX en código, ver `ui-router` ([docs/UI_UX_PRO_MAX_INTEGRATION.md](file:///var/www/html/proyectos/AIPP/jarvis-skills-library/docs/UI_UX_PRO_MAX_INTEGRATION.md)).

Para artefactos visuales marketing (carrusel, deck, email HTML), ver `open-design-router` ([docs/OPEN_DESIGN_INTEGRATION.md](file:///var/www/html/proyectos/AIPP/jarvis-skills-library/docs/OPEN_DESIGN_INTEGRATION.md)) — no `speckit-specify`.

Para prototipos web en Google Stitch (MCP, DESIGN.md, stitch-loop), ver `stitch-router` ([docs/STITCH_UPSTREAM.md](file:///var/www/html/proyectos/AIPP/jarvis-skills-library/docs/STITCH_UPSTREAM.md)) — no sustituye `ui-router` en Flutter/Blade del producto.
...
Para what-if estratégico y simulación multi-agente, ver `scenario-router` ([docs/STRANGEVERSE_INTEGRATION.md](file:///var/www/html/proyectos/AIPP/jarvis-skills-library/docs/STRANGEVERSE_INTEGRATION.md), [docs/MIROFISH_UPSTREAM.md](file:///var/www/html/proyectos/AIPP/jarvis-skills-library/docs/MIROFISH_UPSTREAM.md)) — no `speckit-specify` salvo que el escenario derive en feature.
...
Para harness ECC (hooks, instincts, rules idioma, `ecc consult`), ver `ecc-router` ([docs/ECC_INTEGRATION.md](file:///var/www/html/proyectos/AIPP/jarvis-skills-library/docs/ECC_INTEGRATION.md)) — no sustituye este workflow.

Para auditoría de seguridad profunda read-only (Cyber Neo, 11 dominios, reporte OWASP 2025), ver `cyber-neo-router` ([docs/CYBER_NEO_INTEGRATION.md](file:///var/www/html/proyectos/AIPP/jarvis-skills-library/docs/CYBER_NEO_INTEGRATION.md)) — no sustituye `security` ni este workflow.

Para detección runtime y política escalonada ante spikes (Kalman + agente), ver `kalman-anomaly-router` ([docs/KALMAN_ANOMALY_INTEGRATION.md](file:///var/www/html/proyectos/AIPP/jarvis-skills-library/docs/KALMAN_ANOMALY_INTEGRATION.md)) — no sustituye `cyber-neo` ni este workflow.

Para captura/consolidación de aprendizajes de sesión (scan/wrap-up), ver `learning-loop-router` ([docs/LEARNING_LOOP_INTEGRATION.md](file:///var/www/html/proyectos/AIPP/jarvis-skills-library/docs/LEARNING_LOOP_INTEGRATION.md)) — complemento de `session-learner-ops`, no sustituto.

Para orquestación automática de loops multi-skill (`skill-loop.yml` + CLI), ver `skill-loop-router` ([docs/SKILL_LOOP_INTEGRATION.md](file:///var/www/html/proyectos/AIPP/jarvis-skills-library/docs/SKILL_LOOP_INTEGRATION.md)) — no sustituye `jarvis-core` ni `learning-loop`.

Para pack Addy Osmani (doubt-driven in-flight vs canónico JARVIS), ver `agent-skills-router` ([docs/AGENT_SKILLS_ADDY_INTEGRATION.md](file:///var/www/html/proyectos/AIPP/jarvis-skills-library/docs/AGENT_SKILLS_ADDY_INTEGRATION.md)) — solo `doubt-driven-development` curado; no sustituye `speckit-*`.

Para pack Rezvani/claude-skills (auditoría pre-install vs megapack), ver `claude-skills-router` ([docs/CLAUDE_SKILLS_REZVANI_INTEGRATION.md](file:///var/www/html/proyectos/AIPP/jarvis-skills-library/docs/CLAUDE_SKILLS_REZVANI_INTEGRATION.md)) — solo `skill-security-auditor` curado.

Para gobernanza humana en bucles agénticos (HITL/HOTL, umbrales, terminación), ver `human-in-the-loop-ops` ([docs/LOOP_AI_ECOSYSTEM.md](file:///var/www/html/proyectos/AIPP/jarvis-skills-library/docs/LOOP_AI_ECOSYSTEM.md)) — complementa `git-guardrails-ops` y `skill-loop-router`.
...
Para SD-X (dev + diseño + docs + validate), ver `sdd-x-index` ([docs/SDX_ECOSYSTEM.md](file:///var/www/html/proyectos/AIPP/jarvis-skills-library/docs/SDX_ECOSYSTEM.md)).
...
| Fase | Cadena |
|------|--------|
| Integrar / diagnosticar JARVIS (`init jarvis`) | `project-bootstrap-ops` → [PROJECT_ONBOARDING.md](file:///var/www/html/proyectos/AIPP/jarvis-skills-library/docs/PROJECT_ONBOARDING.md) → OK usuario → Paso A/B/C |
...
| Prototipo web Stitch (MCP, stitch::generate-design, stitch-loop) | `stitch-router` → skills upstream ([STITCH_UPSTREAM.md](file:///var/www/html/proyectos/AIPP/jarvis-skills-library/docs/STITCH_UPSTREAM.md)) |
...
## Overlay clawvis — jarvis-core (holding OpenClaw)

Precedencia y honestidad de `jarvis-core` aplican. Ajustes holding:

1. **Aprobación CEO / AG gates** antes de acciones irreversibles (publish RRSS, deploy, cambios `openclaw.json`).
2. Orquestación por defecto: `jarvis-experts` → `fan-out-synthesize-ops` (no invertir).
