...
/**
 * Resolve workspace paths from openclaw.json (often relative to jarvis-ecosystem or cwd).
 */
async function resolveWorkspaceDir(workspace: string | undefined): Promise<string | undefined> {
  ...
  if (path.isAbsolute(trimmed)) {
    ...
    candidates.push(
      path.join(os.homedir(), "jarvis-ecosystem", rel),
  ...
  for (const dir of candidates) {
    ...
    if (await fileExists(path.join(resolved, "IDENTITY.md"))) return resolved;
    if (await fileExists(path.join(resolved, "SOUL.md"))) return resolved;
  }
  return candidates[0] ? path.resolve(candidates[0]) : undefined;
}
...
export async function GET() {
  ...
  try {
    ...
    for (const agentId of agentIds) {
      ...
      if (workspaceDir) {
        const soulContent = await readTextFile(path.join(workspaceDir, "SOUL.md"));
