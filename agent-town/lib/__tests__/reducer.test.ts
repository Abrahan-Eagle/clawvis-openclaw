...
describe("mergeDiscoveredSeats", () => {
  ...
  it("auto-assigns discovered agents to seats when no stored agentConfig", () => {
    const discovered = [makeDiscoveredSeat({ seatId: "seat-0", index: 0 })];
    const agents = [{ agentId: "jarvis", identity: { name: "Jarvis" } }];
    const result = mergeDiscoveredSeats(discovered, [], [], agents);
    expect(result[0].seatType).toBe("agent");
    expect(result[0].agentConfig?.agentId).toBe("jarvis");
    expect(result[0].label).toBe("Jarvis");
    expect(result[0].assigned).toBe(true);
  });

  it("does not auto-assign when seat is explicitly worker", () => {
    const discovered = [makeDiscoveredSeat({ seatId: "s1", index: 0 })];
    const stored = [makePersistedConfig({ seatId: "s1", seatType: "worker" })];
    const agents = [{ agentId: "jarvis" }];
    const result = mergeDiscoveredSeats(discovered, stored, [], agents);
    expect(result[0].seatType).toBe("worker");
    expect(result[0].agentConfig).toBeUndefined();
  });
});
