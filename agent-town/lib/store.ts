...
import type { AgentConfig, SeatState, TaskItem, GatewayConfig, ChatMessage } from "@/types/game";
...
export function StudioProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const unsub = gameEvents.on("seats-discovered", async (discovered) => {
      let discoveredAgents: AgentConfig[] | undefined;
      try {
        const res = await fetch("/api/agents/discover");
        if (res.ok) {
          const data = (await res.json()) as { agents?: AgentConfig[] };
          if (data.agents?.length) discoveredAgents = data.agents;
        }
      } catch {
        /* ignore — discover optional */
      }
