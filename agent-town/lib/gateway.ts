...
export class GatewayClient {
  ...
  private storeGrantedScopes(frame: GatewayFrame) {
    ...
    const auth = frame.payload?.auth as Record<string, unknown> | undefined;
    const scopes =
      (auth?.scopes as unknown[]) ??
      frame.payload?.scopes ??
      (frame.payload as Record<string, unknown>)?.["scopes"];
    if (Array.isArray(scopes)) {
      for (const s of scopes) {
        if (typeof s === "string") this._grantedScopes.add(s);
      }
    }
    if (this._grantedScopes.size === 0) {
      for (const s of ["operator.read", "operator.write", "operator.admin"]) {
        this._grantedScopes.add(s);
      }
    }
  }
