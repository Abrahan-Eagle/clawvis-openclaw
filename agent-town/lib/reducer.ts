...
export function mergeDiscoveredSeats(
  ...
  return discovered.map((seat, index) => {
    ...
    const agentForSeat = discoveredAgents?.[index];
    const shouldAutoAgent =
      Boolean(agentForSeat) &&
      !stored?.agentConfig &&
      stored?.seatType !== "worker" &&
      stored?.assigned !== false;

    if (shouldAutoAgent && agentForSeat) {
      const label = stored?.label ?? agentForSeat.identity?.name ?? agentForSeat.agentId;
      const seatType: SeatType = "agent";
      const roleTitle = stored?.roleTitle ?? "Independent Agent";
